...
module Stattleship
  RSpec.describe BasketballTopStats do
    describe '#nba_top_stats' do
      it 'returns all the nba_top_stats' do
        expect(nba_top_stats.count).to eq 6
      end
    end
  end
  ...
  RSpec.describe TopStat do
    it 'knows its place' do
      expect(nba_top_stats.first.place).to eq 1
      expect(nba_top_stats.last.place).to eq 2
    end

    it 'knows about a player in first place' do
      player = nba_top_stats.first.player
    ...
    it 'knows about a player in last place' do
      player = nba_top_stats.last.player
