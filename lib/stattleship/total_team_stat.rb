module Stattleship
  class TotalTeamStat < Stattleship::Endpoint
    def to_sentence
      if season_name.nil?
        "The #{team.full_name} have #{StatFormatter.stat(stat_name: stat, value: total)} #{humanized_stat.downcase}"
      else
        "The #{team.full_name} had #{StatFormatter.stat(stat_name: stat, value: total)} #{humanized_stat.downcase} in the #{season_name} season"
      end
    end
